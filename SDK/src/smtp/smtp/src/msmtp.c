...
void msmtp_print_help(void)
{
    printf(_("USAGE:\n\n"
	    ...
	    "  --auth[=(on|off|method)]     Enable/disable authentication and "
	    	"optionally\n"
	    "                               choose the method.\n"
		"  --user=[username:password]   Set/unset user name and password\n"
	    "			       for authentication.\n"
...
int msmtp_cmdline(msmtp_cmdline_conf_t *conf, int argc, char *argv[])
{
	...
	switch(c)
	{
		...
		if(*optarg == '\0')
		{
			...
			conf->cmdline_account->username = NULL;
		}
		else
		{
			...
			{
				*password++ = '\0';
				conf->cmdline_account->password = xstrdup(password);
				
			}
			conf->cmdline_account->username = xstrdup(optarg);
		}
