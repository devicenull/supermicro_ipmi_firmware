...
static void alarm_catcher(int sig UNUSED_PARAM)
{
		struct tsession *ts;
		int min = TIMEOUT+1;        
		ts = sessions; 
        ...
        while (ts)
        {
            //search exist alarm leader
            if (ts->time_to_expire == -1)
			{
				ts->shell_pid = -1;//close sessions
            }
            else if(ts->time_to_expire < min && ts->time_to_expire > 0)
            {   //find minimum expire time among sessions
				min = ts->time_to_expire;
            }
            ts = ts->next;
        }
		ts = sessions;
		while (ts)
        {
			if (ts->time_to_expire == min)
            {
                ts->time_to_expire = -1; //set new alarm leader
            }
            else
                ts->time_to_expire  -= min; //refresh all sessions
            ts = ts->next;
        }
        //fire new timeout alarm
		if(sessions)
		{
			alarm(min);
#ifdef TIMEOUT_DEBUG
			printf("time to next alarm = %d\n", min);
#endif
		}
}
...
make_new_session(
	...
	if (pid > 0) {
		...
		{
#ifdef TIMEOUT_DEBUG
			printf("New session comming in | leader = %d\n", leadtime);
#endif
			if(leadtime == TIMEOUT) //new session will join to lead with other alarm leader
			{
				ts->time_to_expire = -1;
#ifdef TIMEOUT_DEBUG
				printf("New session joins as a alarm leader\n");
#endif
			}
			else
			{
				ts->time_to_expire = TIMEOUT - leadtime;
#ifdef TIMEOUT_DEBUG
				printf("New session joins as a alarm slave\n");
#endif
			}
		}
		else
		{
            ...
            ts->time_to_expire = -1;
#ifdef TIMEOUT_DEBUG
			printf("first session comming in\n");
#endif
		}
		return ts;
	}
...
#define IS_INETD (opt & OPT_INETD)
	...
	/* Set SIGALRM handler to support idle timeout */
...
*/
 ...
 skip2:
		...
		if (/*ts->size1 < BUFSIZE &&*/ FD_ISSET(ts->sockfd_read, &rdfdset)) {
			...
			if(ts->time_to_expire == -1)
			{
				int a_flag = 0, scount = 0;
				int min = TIMEOUT+1;
				
#ifdef TIMEOUT_DEBUG
				printf("trigger is a alarm leader\n");
#endif
				tstmp = ts;
				...
				while (ts)
				{
					scount++;
					//search exist alarm leader
					if (ts->time_to_expire == -1 && ts != tstmp)
					{
						a_flag = 1;
						min = 0;
					}
				...
				if(!a_flag)//no exist alarm leader
				{
					if(min != TIMEOUT + 1)
					{
						ts = sessions;
						...
						alarm(leadtime);
#ifdef TIMEOUT_DEBUG
						printf("change leader timeout = %d %d\n", leadtime, min);
#endif
					}
					else
						alarm(TIMEOUT);//only one session exist, reset its timeout
				}

				if(min == TIMEOUT + 1)//all sessions are alarm leader
				{
					tstmp->time_to_expire = -1;
				}
				else
				{
					tstmp->time_to_expire = TIMEOUT - leadtime;
				}
			...
			{
				//the session been update is not alarm leader, just reset its own timeout
#ifdef TIMEOUT_DEBUG
				printf("reset timeout of alarm slave = %d\n", TIMEOUT - leadtime);
#endif
				ts->time_to_expire = TIMEOUT - leadtime;
				//alarm(leadtime);
			}	
